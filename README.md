...
- Docker Compose：FastAPI、PostgreSQL、Caddy
- SSH Docker Engine：`ssh://nekocon-server`（本机开发时该别名在 `~/.ssh/config` 指向 `127.0.0.1`）

## 架构概览

```text
Browser
  |-- REST /api/* ----------> Caddy ----------> FastAPI routers
  |-- WebSocket /ws --------> Caddy ----------> ConnectionManager
  |-- /uploads/* -----------> Caddy ----------> uploads volume
                                                   |
...
必须删除 `ai-draw_frontend-dist`，否则 Caddy 可能继续提供旧的前端静态文件。该流程只删除前端构建卷，不删除 PostgreSQL、上传文件和 Hugging Face 模型缓存卷。

默认访问地址：

- 线上站点：<https://aidraw.nekocon.cn/>
- Caddy HTTP：`http://127.0.0.1:14601`
- 后端 Swagger：`http://127.0.0.1:14600/docs`
- 后端 ReDoc：`http://127.0.0.1:14600/redoc`
